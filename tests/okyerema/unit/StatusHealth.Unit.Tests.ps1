...
Describe "Get-Sitrep" {
    BeforeAll {
        $scriptPath = Join-Path $healthPath "Get-Sitrep.ps1"
        $mockResponse = @'
{
  "data": {
    "repository": {
...
Describe "Get-HierarchyHealth" {
    BeforeAll {
        $scriptPath = Join-Path $healthPath "Get-HierarchyHealth.ps1"
...
Describe "Get-DagStatus" {
    BeforeAll {
        $scriptPath = Join-Path $rhythmPath "Get-DagStatus.ps1"
...
Describe "Get-DagCompletionReport" {
    BeforeAll {
        $scriptPath  = Join-Path $rhythmPath "Get-DagCompletionReport.ps1"
...
Describe "Invoke-DagHealthCheck" {
    BeforeAll {
        $scriptPath = Join-Path $rhythmPath "Invoke-DagHealthCheck.ps1"
        $fixtureData = Get-Content (Join-Path $fixturesPath "dag-health.json") -Raw
    }
